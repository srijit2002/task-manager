...
const SingleTask = () => {
  ...
  useEffect(() => {
     const [{title,details,dueDate}]=tasks.filter((task)=>(task._id===taskId));
     const newDate=new Date(parseInt(dueDate));
     const month=newDate.getMonth();
     const year=newDate.getFullYear();
     const date=newDate.getDate();
     const formattedNewDate=`${year}-${(month<10)?`0${month}`:month}-${(date<10)?`0${date}`:date}`;
     setTaskTitle(title);
  ...
  const saveEditedTask=async(e)=>{
    e.preventDefault()
    const [{roomCode,_id,isCompleted}]=tasks.filter((task)=>(task._id===taskId));
    ...
    const newTaskModel={_id, details:taskdetails, dueDate:configuredDate, isCompleted, roomCode, title:taskTitle};
    try {
      const newTasks=await axios.patch(`http://localhost:8000/api/v1/task/${userId}`,newTaskModel)
      history.push("/")
      ...
      <TaskCard>
        <Form className="form" onSubmit={saveEditedTask}>
          <div className="form__inputs">
            ...
            <input required type="text" id="title" className="task__title" value={taskTitle} onChange={e=>setTaskTitle(e.target.value)}/>
          </div>
          <div className="form__inputs">
            <label htmlFor="due-date">Due date of the task</label>
            <input type="date" id="due-date" className="task__due-date" value={taskDueDate} onChange={e=>setTaskDueDate(e.target.value)}/>
          </div>
          <div className="form__inputs">
            <label htmlFor="details">Description of the task</label>
            <textarea id="details" className="task__details" value={taskdetails} onChange={e=>setTaskDetails(e.target.value)}/>
          </div>
          <div className="button__wrapper">
            <Button className="btn btn--primary" type="submit">
              Save
            </Button>
            <Button className="btn btn--secondary" onClick={redirectToHomePage}>Cancel</Button>
          </div>
        </Form>
      </TaskCard>
    </PageWrapper>
